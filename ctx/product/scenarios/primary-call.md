...
## Предусловия
- Колобок открыт, камера и микрофон разрешены, интернет работает.

## Основной поток
1. Инициатор нажимает большую кнопку **Позвонить** на домашнем экране. Колобок генерирует свежий UUID-сеанс, очищает URL от параметров и сразу показывает экран приглашения со ссылкой `https://<домен>/?session=<uuid>`.
