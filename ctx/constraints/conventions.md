...
- Только технический идентификатор сеанса налагает границы контекста сеанса, и он единственный носитель идентификационной информации в модели (`ctx/product/terms.md`).
- Документация и конфигурации упоминают исключительно уже утверждённые идентификаторы сеанса, сохраняя согласованность пространства идентификаторов (`ctx/product/terms.md`).

---

## Логирование и метаданные

- Все каналы телеметрии и локального журналирования фиксируют только технические метки (идентификатор сеанса, `role`, `status`) и строго исключают ПДн, отражая абстракцию автономного узла (`ctx/product/overview.md`).
...
- Каждый идентификатор сеанса связывается ровно с двумя участниками, а любые модели с иными количествами сторон выходят за рамки продукта (`ctx/product/capabilities/session.md`).
