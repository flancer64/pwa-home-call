...
- `conventions.md` — постоянные инженерные инварианты: единственный `sessionId`, упрощённый лог-трейс и двухучастниковость.
- `link.md` — инварианты ссылки как декларации `sessionId`, не создающей сеанс до обращения и обрывающей контекст после завершения.
- `node.md` — изоляция контекстов узла: параллельные `sessionId`, изолированные состояния и строгая двухучастниковая модель.
...
- `storage.md` — ограничения хранилища: лишь технические метаданные, отсутствие `sessionId` и недопустимость восстановления контекста.
