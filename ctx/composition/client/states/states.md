...
Путь: `ctx/composition/client/states/states.md`
...
- Использует `ctx/composition/client/ui/layout/skeleton.md` и `screen-card`, чтобы закрепить `screen-header`, один доминирующий `big-button` и `screen-note`.
...
- Footer-зона показывает `screen-note` с поддерживающим текстом; статусы показываются через `ctx/composition/client/ui/patterns/toast-layer.md`.

## Фрагмент `waiting`

- Применяет тот же скелет, но над ним активируется `ctx/composition/client/ui/layout/overlays/share-link.md`.
...
- Прерывает скелет: layout определяется `ctx/composition/client/ui/layout/call.md`, который кладёт `remoteStream` на фон, `localStream` и `ctx/composition/client/ui/patterns/fab-panel.md` над медиапотоком.
...
- Все фрагменты используют палитру и отступы `ctx/composition/client/ui/components/style.md`.
- Overlay`ы шаринга и настроек остаются декларативными, поэтому layout описан отдельно `ctx/composition/client/ui/layout/overlays/share-link.md`.
