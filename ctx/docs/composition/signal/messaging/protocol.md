...
6. **Ошибки/отказы** — `error` рассылаются заранее, чтобы UI мог отобразить сообщение через `ctx/docs/composition/client/ui-elements/notifications.md`.
...
- Сообщения `offer/answer/candidate` не переносят UI-логику — клиент просто отображает `call`-макет и тосты, описанные в `ctx/docs/composition/client/ui-states/states.md`.
