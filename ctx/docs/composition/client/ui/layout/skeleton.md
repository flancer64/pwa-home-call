...
- Корневые контейнеры (`screen-shell`, `screen-card`) применяют `env(safe-area-inset-*)` для минимум 16px отступов, которые защищают зоны от вырезов и системных панелей; детали инварианта фиксируются здесь.
