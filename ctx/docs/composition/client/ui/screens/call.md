...
Сцена опирается на два видеослота с нейтральными названиями: `main-video` отмечает основной фон и растягивается на всю сцену, а `overlay-video` — малый оверлей `call-local`, куда переселяется превью собственного потока после установления соединения, когда основной слот передаёт соседний видеофид.
...
- основной экран показывает локальное видео в `main-video`, а `overlay-video` остаётся скрытым и без кадра;
...
- основной экран (`main-video`) начинает отображать удалённый видеофид;
- локальное видео появляется в малом оверлее (`overlay-video`) внутри контейнера `call-local`;
