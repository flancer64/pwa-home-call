...
Path: `./ctx/docs/environment/pwa.md`
...
> **Согласованность с продуктом:** `ctx/docs/product/overview.md` подчеркивает независимость одного окна, поэтому PWA-кэш содержит только файлы, необходимые для логики `ready/waiting/active`, и никак не хранит дополнительные экраны или персональные данные. Документ описывает текущее поведение, а будущая интеграция state machine с PWA перечислена в `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md`.
...
Этот документ перенесён из `ctx/docs/architecture/web/infra/pwa.md` в рамках нормализации уровней ADSM.
