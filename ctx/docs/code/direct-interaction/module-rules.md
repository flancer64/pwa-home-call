...
- `HomeCall_Web_Media_Manager$` запрашивает доступ к `navigator.mediaDevices`, при отсутствии API возвращает читаемый warning и не блокирует приложение. Потоки хранятся в памяти, а методы `setLocalStream`, `prepare`, `getLocalStream` обновляют `Peer`; до подключения удалённого участника локальный поток вставляется в DOM элементы (`main-video`, overlay) как `muted`, автоплей звука запрещён и переключение на remote stream происходит только после `subscribeRemoteStream`.
- `HomeCall_Web_Rtc_Peer$` инициализирует `RTCPeerConnection` через `env.RTCPeerConnection`, добавляет обработчики `track`, `icecandidate`, `connectionstatechange`, логирует изменения с префиксом `[Peer]` и предоставляет методы подписки на `remoteStream`; он назначает `remoteStream` в `main-video` с включённым звуком и не направляет локальные треки на аудиовыход.
