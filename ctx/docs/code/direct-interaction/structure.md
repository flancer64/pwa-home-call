...
- `web/app/Media/Manager.mjs` и `Media/Monitor.mjs` управляют потоками `MediaStream`, отслеживают статус микрофона/камеры и публикуют события через DI; менеджер вставляет локальный поток в DOM как muted и не запускает аудио до появления удалённого потока.
- `web/app/Rtc/Peer.mjs` обёртывает `RTCPeerConnection`, подписывается на `track`, `icecandidate` и предоставляет методы `sendOffer`, `sendAnswer`, `sendCandidate`, `subscribeRemoteStream`; `subscribeRemoteStream` назначает поток в `main-video` с включённым звуком и не подмешивает локальные треки.
