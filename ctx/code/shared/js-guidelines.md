...
Path: `./ctx/code/shared/js-guidelines.md`

Эти правила определяют, как пишутся JavaScript-модули и инфраструктура во всех контурах, включая слабые места, где работает Codex. Они дополняют архитектуру (`ctx/architecture/overview.md`) и продукт (`ctx/product/overview.md`), но не переопределяют их.
...
- Все классические `Codex`-агенты должны опираться на эти нормы: `ctx/code` — технологическая опора для генерации модулей, управление кодом и проверка `npm run lint`/`npm run test:*`.
