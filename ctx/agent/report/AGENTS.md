...
- `2025/` — архив итерационных отчётов, организованных по годам, месяцам и дням.
- `AGENTS.md` — текущий файл, описывающий требования к отчётности и структуре каталога `report/`.
...
- Каталог `ctx/agent/report/` является только для записи: без прямого указания человека агент не открывает, не модифицирует и не просматривает уже существующие файлы; архивы не читаются и остаются в прошлом.
...
- Агент не должен читать предыдущие отчёты без прямого указания, т.к. отчёты предназначены для людей и храняться для истории. К текущим задачам отчёты обычно отношения не имеют.
