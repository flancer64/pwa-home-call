...
> **Согласованность с продуктом:** `ctx/product/overview.md` требует одно окно `ready/waiting/active`, поэтому Codex должен избегать scaffold-ов, создающих отдельные экраны `home → invite → call → end` и вместо этого учитывать, что новая state machine описана в `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md`. Текущая реализация ещё использует экраны, но Codex не генерирует дополнительных поведенческих веток для приглашений или комнат.
