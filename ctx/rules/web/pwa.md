...
## 5. Ручная очистка кэша

- На экране `enter` добавлена кнопка `Очистить кэш`, которая находится рядом с основными действиями (например, `Подготовить камеру`) и показывает сообщение `Кэш удалён. Приложение будет перезапущено` перед перезагрузкой.
- При нажатии UI отправляет событие `ui:action:clear-cache`, которое перехватывает Core и делегирует `HomeCall_Web_Pwa_CacheCleaner$`. Модуль запрашивает `caches.keys()`, удаляет каждый кеш (`caches.delete`) и вызывает `location.reload()`, чтобы инициировать новую сборку кэша текущей версии (`homecall-v{version}`).
- При ошибках или ручной очистке модуль также выполняет `navigator.serviceWorker.getRegistrations()` и `registration.unregister()` для всех активных сервис-воркеров, чтобы перезапуск начинался с «чистого листа» и новые ресурсы заново заносятся в кэш.
- Благодаря ручной очистке пользователь может сбросить все версии кэша, созданные `service-worker.js`, и сразу увидеть свежую сборку приложения.
