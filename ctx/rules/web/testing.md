...
Документ определяет правила модульного тестирования **браузерной части** проекта **ДомоЗвон**.
...
- Все тесты веб-части обязаны создавать контейнер через функцию `createWebContainer()` из `test/web/helper.mjs`.

- Запрещено использовать **статические импорты** проектных модулей, например:

  ```js
  import HomeCall_Web_Ui_Toast from "../../../../web/app/Ui/Toast.mjs"; // ❌ запрещено
  ```

- Допускаются только импорты:

  - стандартных тестовых библиотек (`node:test`, `node:assert/strict`, `sinon` и аналогичных);
  - вспомогательных модулей тестовой инфраструктуры (`helper.mjs`, фикстуры, утилиты).

- Все компоненты приложения должны загружаться **через контейнер зависимостей**:
...
- Такое требование обеспечивает воспроизводимость среды, изоляцию тестов и архитектурную согласованность с DI-моделью TeqFW.
...
Документ устанавливает единообразные принципы тестирования веб-компонентов **ДомоЗвон**.
