...
> **Согласованность с продуктом:** `ctx/product/overview.md` и `ctx/product/capabilities/connection.md` предполагают состояние `ready → waiting → active` без завязки на отдельные экраны, поэтому тесты должны моделировать переходы между этими состояниями даже если UI продолжает использовать `home`, `invite`, `call`, `end`. Будущая трансформация state machine описана в `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md`, но диаграмма `ready/waiting/active` пригодна уже сейчас для проверки логики.
