...
Документ задаёт принципы работы браузерного приложения Колобок с локальными хранилищами (`localStorage`, `IndexedDB` и аналогичными механизмами).  
...
2. Все записи оформляются как отдельные ключи (`kolobok:lastVersion`, `kolobok:flags`); их формат JSON позволяет безопасно расширять набор полей без ломки прежних сборок.  
...
Локальное хранилище Колобок теперь обслуживает только PWA-метаданные и флаги.  
