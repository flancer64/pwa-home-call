...
4. Домашний экран также предлагает кнопки **«Изменить имя»** (вызов `setMyName(null)`) и **«Очистить кэш»** (вызов `resetMyData()`, затем запуск `HomeCall_Web_Pwa_CacheCleaner$`), чтобы показать пользователю, что можно очистить локальное имя, кэш и зарегистрированные сервис-воркеры без покидания интерфейса.
5. Очистка данных инициируется системно (например, через настройки браузера или жесты PWA) и возвращает приложение к состоянию «первого запуска».
...
- Локальные данные не очищаются вместе с кэшем по решению пользователя или в ходе обновления PWA, если это предусмотрено политикой версии.
