...
Документ задаёт принципы работы браузерного приложения Приложение с локальными хранилищами (`localStorage`, `IndexedDB` и аналогичными механизмами).  
...
> **Согласованность с продуктом:** `ctx/product/overview.md` и `ctx/product/capabilities/connection.md` фиксируют, что хранилище должно содержать только PWA-метаданные и флаги `ready/waiting/active`, без сохранения `sessionId` или ссылок. Документ описывает текущую реализацию, а `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md` объясняет, как future state machine будет работать с этими данными в памяти, не записывая их на диск.
...
Приложение остаётся полностью работоспособным при полном отсутствии доступа к хранилищу; ключевые сценарии формируются динамически.
...
Локальное хранилище Приложение теперь обслуживает только PWA-метаданные и флаги.  
