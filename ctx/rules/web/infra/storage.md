...
4. Домашний экран также предлагает кнопки **«Изменить имя»** (вызов `setMyName(null)`) и **«Сбросить настройки»** (вызов `resetMyData()`), чтобы показать пользователю, что данные можно очистить без покидания интерфейса.
