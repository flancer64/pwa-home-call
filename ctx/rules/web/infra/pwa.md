# Прогрессивное веб-приложение Связист

Документ определяет декларативную архитектуру прогрессивного веб-приложения (PWA) Связист.  
...
Система PWA Связист обеспечивает:
...
2. Имя кэша определяется как `svyazist-v{version}`.  
...
3. Обновление версии — управляемый асинхронный цикл:
   ...
   - сервис-воркер очищает все кэши, пересоздаёт кэш `svyazist-v{version}` и снова загружает Core Assets без перерегистрации;
...
2. При установке и активации воркер:
   ...
   - создаёт кэш `svyazist-v{version}` и загружает набор Core Assets;
...
3. Обновление версии происходит без перерегистрации воркера: `VersionWatcher` присылает сообщение `clear-caches-and-rebuild`, и воркер:
   ...
   - создаёт новый кэш `svyazist-v{version}` и записывает в него Core Assets;
...
4. Очистка кэша инициируется только через сигнал `clear-caches-and-rebuild`: либо `VersionWatcher`, либо сервис-воркер направляют команду `HomeCall_Web_Pwa_CacheCleaner$`.  
   Компонент удаляет все `CacheStorage`, сбрасывает встроенное хранилище (в том числе флаги/метки `lastVersion`), отменяет регистрацию сервис-воркеров и перезагружает клиент.  
   Если пользователь вручную очищает данные сайта или удаляет PWA, браузер вызывает тот же `CacheCleaner`, оставаясь в рамке одной и той же процедуры.
...
1. В интерфейсе `home` теперь нет кнопки **«Очистить кэш»** — очистку запускают только системные агенты или средства браузера (`Очистить данные сайта`, `Сбросить PWA`, удаление приложения).  
2. Все такие сценарии направляют команду в `HomeCall_Web_Pwa_CacheCleaner$`, который:
   - удаляет все `CacheStorage` и перезаписывает `svyazist-v{version}`;
   - сбрасывает хранилище (флаги, версии, метки) без упоминания `sessionId` или пользовательских данных;
   - логирует `"[CacheCleaner] PWA cache cleared and service worker reinstalled"` и перезагружает страницу.
3. Если очистка завершается с ошибкой, клиент показывает `toast.error('Не удалось очистить кэш. Обновите страницу вручную.')`, и после перезапуска приложение продолжает работу с актуальными метаданными.
...
- Каждая версия фронтенда зависит от собственного кэша `svyazist-v{version}`, но работает в рамках неизменного сервис-воркера.
...
Архитектура PWA Связист обеспечивает предсказуемое, наблюдаемое и воспроизводимое поведение при изменении версии.  
