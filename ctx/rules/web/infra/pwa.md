...
4. Очистка кэша выполняется либо в рамках автоматического обновления версии (через сообщение `clear-caches-and-rebuild` и сам сервис-воркер), либо по явному запросу пользователя.  
   `HomeCall_Web_Core_CallFlow` добавляет кнопку **«Очистить кэш»** на `home`: она вызывает `storage.resetMyData()`, обновляет локальный контекст и поручает `HomeCall_Web_Pwa_CacheCleaner$` удалить все кэши, отменить регистрации сервис-воркеров и перезагрузить страницу.  
   Браузерные средства («Очистить данные сайта», «Сбросить PWA» и т.п.) по-прежнему запускают тот же `CacheCleaner`, даже если пользователь работает вне UI ДомоЗвона.
...
1. В интерфейсе ДомоЗвон есть кнопка **«Очистить кэш»** на `home`. Она сигнализирует, что пользователь может вернуть приложение к состоянию первого старта не только очистив имя, но и сбросив весь PWA-кэш.
2. При нажатии `HomeCall_Web_Core_CallFlow`:
   - вызывает `storage.resetMyData()` и очищает локальный контекст (`pendingRoom`, `myName`, `activeRoom` и связанное состояние);
   - показывает `toast.info('Очистка кэша и локального хранилища...')`, а затем вызывает `HomeCall_Web_Pwa_CacheCleaner$`, который удаляет все `CacheStorage`-записи, отменяет регистрации сервис-воркеров, логирует `"[CacheCleaner] PWA cache cleared and service worker reinstalled"` и перезагружает страницу;
   - при сбое `CacheCleaner` появляется `toast.error('Не удалось очистить кэш. Обновите страницу вручную.')`, а интерфейс остаётся в начальном состоянии.
3. Эта операция дополняет автоматические обновления — пользователь получает кнопку для мануального сброса, но слой `VersionWatcher` по-прежнему сам отправляет сообщения `clear-caches-and-rebuild`.
