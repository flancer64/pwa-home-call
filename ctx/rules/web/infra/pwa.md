# Прогрессивное веб-приложение Колобок

Документ определяет декларативную архитектуру прогрессивного веб-приложения (PWA) Колобок.  
...
Система PWA Колобок обеспечивает:
...
2. Имя кэша определяется как `kolobok-v{version}`.  
...
3. Обновление версии — управляемый асинхронный цикл:
   ...
   - сервис-воркер очищает все кэши, пересоздаёт кэш `kolobok-v{version}` и снова загружает Core Assets без перерегистрации;
...
2. При установке и активации воркер:
   ...
   - создаёт кэш `kolobok-v{version}` и загружает набор Core Assets;
...
3. Обновление версии происходит без перерегистрации воркера: `VersionWatcher` присылает сообщение `clear-caches-and-rebuild`, и воркер:
   ...
   - создаёт новый кэш `kolobok-v{version}` и записывает в него Core Assets;
...
2. Все такие сценарии направляют команду в `HomeCall_Web_Pwa_CacheCleaner$`, который:
   - удаляет все `CacheStorage` и перезаписывает `kolobok-v{version}`;
...
- Каждая версия фронтенда зависит от собственного кэша `kolobok-v{version}`, но работает в рамках неизменного сервис-воркера.
...
Архитектура PWA Колобок обеспечивает предсказуемое, наблюдаемое и воспроизводимое поведение при изменении версии.  
