...
1. При запуске `HomeCall_Web_Core_App` устанавливает соединение и отправляет `room` при генерации ссылки.
2. Инициатор после нажатия «Позвонить» отправляет `offer`, слушает `answer` и `candidate`, которые поступают от получателя.
3. Получатель, открыв ссылку, подключается к той же комнате, принимает `offer` и отвечает `answer` с собственным `candidate`.
4. Все ошибки возвращаются как сообщения `"error"` — Core использует их для `toast.error()` и, при необходимости, переподключения.
5. Нет событий `online`, `join`, `leave` или списков пользователей — WebSocket лишь проксирует сигналы для конкретной комнаты.

---

## 4. Принципы реализации

- Сервер не модифицирует SDP/ICE, а лишь ретранслирует для участников той же комнаты.
- Повторные подключения идентичны: `room` фиксирован, и повторный `offer`/`answer` служит возобновлению.
- Сообщения логируются через `HomeCall_Web_Shared_Logger$`; состояния сети отражаются только в `toast`.
