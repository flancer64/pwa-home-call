...
> **Согласованность с продуктом:** `ctx/product/overview.md` и `ctx/product/capabilities/connection.md` требуют, чтобы сигналинг работал только внутри состояний `waiting → active` одного окна и не хранил личные идентификаторы. Документ фиксирует текущую реализацию, где `home`/`invite` подготавливают `sessionId`, а план `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md` показывает, как именно `Net` и `signaling` будут сочетаться с будущей inline-state machine.
...
| Тип            | Поля                                    | Назначение                           |
| -------------- | --------------------------------------- | ------------------------------------ |
| `"offer"`      | `sessionId`, `sdp`                      | Передача SDP-предложения             |
| `"answer"`     | `sessionId`, `sdp`                      | Ответ на предложение                 |
| `"candidate"`  | `sessionId`, `candidate`                | Обмен ICE-кандидатами                |
| `"error"`      | `sessionId`, `message`, `code?`         | Уведомление об ошибке сигналинга     |

Каждое сообщение содержит `sessionId`, позволяя серверу поддерживать FIFO-очереди SDP/ICE и гарантировать, что данные приходят только участникам одной сессии.
