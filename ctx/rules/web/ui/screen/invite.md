...
2. **Main** — блок ссылки с `component/big-button` (Share/Copy) и `component/big-button` **«Связать»**; действия `share`/`copy` описаны в `share-link.md`.
3. **Footer** — `component/screen-note` для короткой подсказки («Скопируйте ссылку и отправьте») или согласованного напоминания; в зоне `footer` он остаётся статичным и не управляется событиями.
...
- Статусы и ошибки выводятся только через `notifications.md`, нижняя зона `footer` остаётся статичной и не дублирует события.
