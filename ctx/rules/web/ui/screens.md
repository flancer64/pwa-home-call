...
2. **lobby** — ожидание собеседников, выбор участника и кнопки выхода.
3. **call** — активная видеосессия, отображение потоков и повторные запросы устройств.
4. **end** — подведение итогов вызова и возврат к лобби.
...
- **enter → lobby:** на успешном соединении с сигнализацией и сохранении данных приложение переходит в лобби, обновляя контекст toolbar и сбрасывая сообщение `connectionMessage`.
...
- `ctx/product/scenarios/share-and-join.md` — сценарий, который порождает URL-параметры `room`/`name`, и продуктовый документ `ctx/product/features.md` (секция `features/share.md`) по совместному подключению.
