...
2. **invite** — заголовок, ссылка и две `component/big-button`: основной (Share/Copy) и CTA **«Связать»**; экран остаётся в рамках `component/screen-card`.
...
- `invite → call` — `component/big-button` **«Связать»** вызывает `onStartCall`, Core берёт на себя подготовку медиа, signal и WebRTC, после чего UI отображает `call`; статусы идут через toast.
...
- `settings` открывается как overlay поверх текущего состояния через кнопку в правой части `header`; она не создаёт новых экранов и возвращает пользователя обратно после закрытия.
