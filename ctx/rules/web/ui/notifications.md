...
Все статусы, ошибки и короткие подтверждения в Приложении передаются исключительно через тосты. Это позволяет экранам оставаться чистыми, не нарушает трёхзонную структуру и соответствует ограничению — никаких оверлеев, дополнительных панелей или описаний поверх видео.

> **Согласованность с продуктом:** `ctx/product/overview.md` требует единое окно и прозрачные состояния `ready/waiting/active`, поэтому уведомления выводятся только через тосты, оставаясь независимыми от `home`, `invite`, `call`, `end`. Этот документ подчёркивает, что никакие дополнительные экранные слои не создаются, даже если текущая реализация использует отдельные экраны, а будущая трансформация описана в `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md`.
