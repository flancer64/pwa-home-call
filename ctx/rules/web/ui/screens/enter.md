...
Примечание: получатель приглашения всегда начинает с экрана `enter`. Параметры `?room` и `?name` из ссылки предзаполняются в форме; экран `invite` получателем не используется.
