...
## Purpose
Документ описывает, как Связист превращает один тап в общее приглашение и сохраняет экран `invite` минималистичным, даже если Web Share API недоступен.
...
- При открытии ссылки Связист читает параметр `session` и передаёт его Core.  
