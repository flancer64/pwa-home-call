...
## Purpose
Документ описывает, как Колобок превращает один тап в общее приглашение и сохраняет экран `invite` минималистичным, даже если Web Share API недоступен.
...
- При открытии ссылки Kolobok читает параметр `session` и передаёт его Core.  
