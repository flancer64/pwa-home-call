...
> **Согласованность с продуктом:** `ctx/product/capabilities/link-sharing.md` требует, чтобы ссылка создавалась внутри уже работающего сеанса и оставалась привязанной к нему. Текущая логика задействует экран `invite` и отдельный CTA, но будущее состояние `waiting`/`active` с inline-линком описано в `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md`; документ сохраняет актуальную реализацию и добавляет ссылку на план.
