...
| **invite** | Экран инициатора для формирования приглашения (ввод имени приглашённого и целевой комнаты, генерация/отправка ссылки). |
...
- Базовый маршрут: `enter → lobby → call → end → lobby`.
- Переход в `invite` — результат ручного действия `ui:action:share-link` из меню или toolbar; Core открывает его независимо от текущего состояния и возвращает инициатора назад, чтобы основной маршрут оставался без изменений.

### Поведение при запуске

- При старте приложение проверяет параметры URL (`room`, `name`).
- Если найден хотя бы один параметр, значения подставляются на `enter`, где получатель подтверждает или редактирует их и выполняет подключение; экран `invite` не задействуется в этом сценарии.
- Если параметры отсутствуют, пользователь проходит обычный путь: `enter → lobby → call`.
- Экран `invite` не активируется автоматически: он вызывается вручную из любого состояния интерфейса через `ui:action:share-link` и не влияет на автоподключение по ссылке.

### Основной и альтернативный потоки

- Основной поток — обычный вход без приглашения: пользователь указывает `room` и своё `name` на экране `enter`, затем попадает в `lobby` и начинает звонок (`call`).
- Побочный поток приглашения стартует вручную: инициатор запускает действие «Поделиться ссылкой» (`ui:action:share-link`) из любого меню или toolbar и переходит на `invite` для генерации ссылки. После отправки инициатор возвращается в исходное состояние и продолжает основной поток без смены комнаты.
- Получатель не видит `invite`: он открывает ссылку, попадает на `enter` с предзаполненными `room`/`name` и далее проходит `lobby → call → end → lobby`.

```
Основной поток: enter → lobby → call → end → lobby
Побочный поток (invite): (enter|lobby|end) —[ui:action:share-link]→ invite → назад → продолжение основного потока
```

Функция «Поделиться ссылкой» вручную запускается только инициатором и не входит в базовый сценарий `enter → lobby → call → end → lobby`.
...
Связанные документы по сценарию приглашения:
