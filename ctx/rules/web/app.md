# Браузерное приложение проекта Колобок

## Назначение

Документ описывает поведение браузерного приложения Колобок как единой системы.  
...
1. **Цикл `home → invite → call → end`.**  
   Колобок сразу создаёт UUID-сеанс, показывает экран приглашения и лишь затем запускает WebRTC. `home` всегда готовит пользователя к звонку одной кнопкой, `invite` демонстрирует ссылку и кнопки копирования/поделения, `call` управляет медиапотоками, `end` завершает сеанс.
