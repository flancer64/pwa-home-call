...
- Хранит имя один раз в `localStorage`, но предоставляет быстрые кнопки **«Изменить имя»** и **«Сбросить настройки»** на `home`.
...
3. **Имя однажды, но легко сбрасывается.**  
   После первого сохранения `home` показывает баннер «Используется имя: <name>», кнопки **«Изменить имя»** / **«Сбросить настройки»**, и выдаёт toast (например, «Имя очищено. Введите новое имя, чтобы продолжить»), чтобы пользователь понимал, что можно вернуться к форме.
...
| --------- | ---------- |
| **home**  | Сбор имени или показ сохранённого имени, кнопка **«Позвонить»**, кнопки **«Изменить имя»** / **«Сбросить настройки»**, баннер «Используется имя: <name>» и подсказка «Вас пригласили в комнату…» при `room`-параметре. |
