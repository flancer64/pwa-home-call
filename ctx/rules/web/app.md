# Браузерное приложение проекта Приложение

## Назначение

Документ описывает поведение браузерного приложения Приложение как единой системы.  
...
- Генерирует новый `sessionId` при нажатии **«Связать»**, подключает генератор ссылок (`share-link`), формирует `/?session=<uuid>` и показывает экран `invite`, оставаясь в `home` до начала звонка.
- Управляет переходом в `call`, когда пользователь нажимает **«Связать»** или когда визит по приглашённой ссылке содержит `session`-параметр: Core готовит медиапотоки, WebRTC и сигналинг, а затем прокидывает команды Ui/Media/Net.
...
sessionId не сохраняется в постоянной памяти.  
Он существует только в оперативном состоянии контуров  
и может извлекаться из адресной строки при наличии параметра.
...
1. **Цикл `home → invite → call → end`.**  
   Приложение сразу создаёт UUID-сеанс, показывает экран приглашения и лишь затем запускает WebRTC. `home` всегда готовит пользователя к звонку одной кнопкой, `invite` демонстрирует ссылку и кнопки копирования/поделения, `call` управляет медиапотоками, `end` завершает сеанс.
2. **Домашний экран без форм.**  
   `home` содержит один крупный CTA **«Связать»** и текст «Ссылка создаётся автоматически». Нет полей ввода, нет хранения имени — вся необходимая информация появляется динамически и исчезает после завершения сеанса.
3. **Экран приглашения.**  
   После генерации `sessionId` `invite` показывает ссылку `https://<домен>/?session=<uuid>`, два действия (**«Скопировать ссылку»**, **«Поделиться»**) и кнопку **«Связать»**. Пользователь может отправить ссылку, а затем самостоятельно инициировать WebRTC.
