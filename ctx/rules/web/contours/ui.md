...
Этот документ описывает контур **Ui**, реализующий путь `home → call → end` без toolbar, панелей или EventBus.
...
- **Входные команды**: `Ui.showHome(name)`, `Ui.showCall(stream, callbacks)`, `Ui.showEnd(connectionMessage)`.
- **Обратные коллбэки**: `onCallRequest`, `onEndRequest`, `onReturn`, `onNameUpdate`.
- **Состояния**:
  - `home` — имя вводится один раз; кнопка `Позвонить` вызывает `onCallRequest`;
  - `call` — показывает `remoteStream`, `#local-miniature`, и кнопку `Завершить`; дополнительные подсказки/статусы отсутствуют;
  - `end` — один итоговый экран с кнопкой `Главный экран`.
...
- `ctx/rules/web/app.md` — состояния `home → call → end`.
...
Контур Ui остаётся минимальным: только `home`, `call`, `end`, без панелей, встроенных статусов и EventBus. Все статусы отображаются через `toast` по указаниям Core.
