...
- **Состояния**:
- `home` — имя вводится один раз; кнопка **«Позвонить»** вызывает `onCallRequest`; новые кнопки **«Изменить имя»** и **«Очистить кэш»** очищают хранилище, сбрасывают клиентский кэш через `HomeCall_Web_Pwa_CacheCleaner$`, перезагружают страницу и возвращают форму, а входящее сообщение говорит «Вас пригласили в комнату …».
