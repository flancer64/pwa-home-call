...
> **Согласованность с продуктом:** `ctx/product/capabilities/connection.md` накладывает `ready`, `waiting`, `active` как состояния, в которых готовится локальный поток, показывается ссылка и выводится удалённое видео; Media обслуживает эти состояния через `onDevicesReady` и отправку `localStream`, пока фронтенд всё ещё использует `home`, `invite`, `call`, `end`. План `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md` объясняет, как DI-интерфейсы будут работать с новым state machine.
