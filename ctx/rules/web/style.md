...
Документ фиксирует минимальные требования к визуальному оформлению фронтенда **Колобок**.  
...
Оформление ограничено рамками браузерного приложения Колобок и не затрагивает серверную часть или детали реализации контуров, описанных в `app.md`.
...
Колобок не содержит отдельного экрана разрешений и не показывает собственные CTA-кнопки вроде «Подготовить камеру» или «Разрешить доступ». При первом запуске приложение коротко объясняет необходимость камеры и микрофона (на 2–3 строках) и сразу вызывает системный диалог браузера (`getUserMedia()`).
...
Интерфейс Колобок предназначен для пожилых людей, использующих смартфоны и компьютеры.  
Основные принципы:
...
Этот документ задаёт обновлённые принципы визуального оформления Колобок, включая UX-сценарии получения разрешений на камеру и микрофон.  
