...
Документ устанавливает конкретные правила модульного тестирования **Node.js-части** проекта **Связист**.  
...
> **Согласованность с продуктом:** `ctx/product/overview.md` обязывает сервер хранить только технический `sessionId` и состояние `ready/waiting/active`; тесты должны имитировать именно эти переходы и удостоверяться, что никакие профили или накопленные приглашения не сохраняются. Будущий single-window state machine описан в `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md`.
...
Документ определяет стандарты модульного тестирования Node.js-компонентов **Связист**.
