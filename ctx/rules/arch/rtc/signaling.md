...
> **Согласованность с продуктом:** `ctx/product/overview.md` и `ctx/product/capabilities/connection.md` требуют, чтобы сигналинг работал в рамках одного окна (`ready/waiting/active`) и обслуживал лишь текущий `sessionId`, не создавая отдельных комнат или invite-экранов. Сервер уже хранит только техническое состояние, а фронтенд пока отображает `home → invite → call → end`; будущая реорганизация в одно окно описана в `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md`.
