...
> **Согласованность с продуктом:** `ctx/product/capabilities/connection.md` задаёт состояния `ready`, `waiting`, `active` внутри одного окна, от которых зависит порядок подготовки локального потока (`ready`) и публикации ссылки (`waiting`) до активации удалённого потока (`active`). В настоящей документации интерфейс всё ещё представлен как `home → invite → call → end`, поэтому медиапоток описывает эти этапы, но будущий код будет эвлюироваться по плану `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md` для выравнивания state machine.
