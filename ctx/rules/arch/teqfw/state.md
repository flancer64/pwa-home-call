...
> **Согласованность с продуктом:** `ctx/product/capabilities/connection.md` задаёт реальные состояния `ready`, `waiting`, `active`, и шаблон состояния должен поддерживать эти названия, даже если фронтенд пока использует `home → invite → call → end`. План `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md` описывает то, как объект состояния свяжется с новой state machine в одном окне.
