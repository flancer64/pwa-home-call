...
> **Согласованность с продуктом:** согласно `ctx/product/overview.md`, интерфейс остаётся понятным и минималистичным, потому никакие дополнительные экраны, персональные данные или постоянные идентификаторы не попадают в строки логов. Логика `home → invite → call → end` пока сохраняется, но будущий переход к `ready/waiting/active` в одном окне (`ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md`) требует, чтобы логгер оставался agnostic к интерфейсу и продолжал записывать только техническую информацию (sessionId, role, status) во всех средах.
