...
> **Согласованность с продуктом:** `ctx/product/overview.md` подчёркивает простоту и автономность, поэтому Apache только возвращает статику и проксирует `/api` и `/signal`, не сохраняя приглашения или имена. Текущий frontend всё ещё варьирует `home → invite → call → end`, но Apache-конфигурация старается сохранять один URL и минимальное состояние `sessionId`, а будущая интеграция с `ready/waiting/active` описана в `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md`.
