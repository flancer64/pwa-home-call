...
Apache используется как внешний HTTP-сервер в архитектуре проекта **ДомоЗвон**: раздаёт статику фронтенда и проксирует API-запросы к Node.js-приложению, построенному на `@flancer32/teq-web` поверх `@teqfw/di`.
...
Документ фиксирует роль Apache как внешнего слоя развёртывания и поддерживает связь с архитектурными декларациями ДомоЗвон.
