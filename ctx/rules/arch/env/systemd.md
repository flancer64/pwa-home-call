...
Фиксирует состояние, при котором backend-приложение **HomeCall** автоматически запускается при загрузке Ubuntu и управляется пользователем через `systemd` с ограниченными правами `sudo` без запроса пароля.
...
Документ фиксирует требуемое состояние интеграции backend-приложения HomeCall в систему Ubuntu:
