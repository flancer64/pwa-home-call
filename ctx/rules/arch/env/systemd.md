...
> **Согласованность с продуктом:** `ctx/product/overview.md` требует автозапуска минимального сервиса без хранения персональных данных, поэтому `svyazist.service` запускается с контролируемыми переменными окружения и пишет логи только в указанное хранилище (`/home/{user}/store/svyazist/log`). Передача `sessionId`, `ready/waiting/active` и других сигналов происходит внутри этого процесса без дополнительных баз данных; будущий переход UI на single-window state machine зафиксирован в `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md`.
