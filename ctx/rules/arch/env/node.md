...
Node.js-окружение обеспечивает выполнение серверной части **Связист**, включая API и канал сигналинга WebSocket.  
...
> **Согласованность с продуктом:** `ctx/product/overview.md` и `ctx/product/capabilities/connection.md` напоминают, что backend должен быть прозрачным, удерживая только технический `sessionId` и статус `ready/waiting/active`, а не профили. Текущая реализация ещё описывает экраны `home → invite → call → end`, но Node.js-окружение уже остаётся stateless, поэтому план `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md` фиксирует будущее сопряжение между frontend-state-machine и этим окружением.
...
Node.js-окружение Связист определяет исполняемую среду для HTTP-API и сигналинга WebRTC.  
