...
Документ описывает систему конфигурации backend-приложения **HomeCall** и перечень параметров, задаваемых через файл `.env`.  
...
Файл `.env` является единственным источником параметров окружения для backend HomeCall.  
