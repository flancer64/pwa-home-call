...
> **Согласованность с продуктом:** `ctx/product/overview.md` обязывает сервер хранить только технические метки (`sessionId`, WebPush-метаданные) без профилей, а `ctx/product/capabilities/connection.md` уточняет, что `sessionId` привязан к текущему окну (`ready/waiting/active`). Текущая реализация всё ещё описывает `home → invite → call → end`, поэтому конфигурация настраивает порты и хосты только для единого WebSocket-сигналинга без дополнительных комнат; будущий переход к единому state machine закреплён в `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md`.
