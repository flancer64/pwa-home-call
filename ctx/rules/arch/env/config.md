...
Документ описывает систему конфигурации backend-приложения **ДомоЗвон** и перечень параметров, задаваемых через файл `.env`.  
...
Файл `.env` является единственным источником параметров окружения для backend ДомоЗвон.  
