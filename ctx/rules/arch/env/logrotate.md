...
> **Согласованность с продуктом:** `ctx/product/overview.md` и `ctx/product/capabilities/connection.md` гарантируют, что никакие персональные данные не попадают в логи и что сервер хранит только технические метки `sessionId`. Поэтому правила ротации ограничены одной директорией (`/home/{user}/store/app/svyazist/log`), которая содержит только временные тектовые записи, легко переносимые между инстансами; будущий переход frontend на единый single-window state machine описан в `ctx/agent/plan/2025/11/20251129-rules-implementation-fixes.md`.
