# Backend ДомоЗвон (`ctx/rules/arch/back.md`)

## Назначение

Документ фиксирует архитектуру серверной части **ДомоЗвон** — совокупность Bootstrap-скрипта (`bin/server.js`) и головного модуля приложения (`HomeCall_Back_App`).  
...
Backend ДомоЗвон включает встроенный сервер **WebSocket-сигналинга**, обеспечивающий обмен SDP и ICE-сообщениями между клиентами WebRTC.  
...
Backend ДомоЗвон — однопроцессная система, в которой Bootstrap-скрипт формирует инфраструктуру, а головной модуль управляет логикой и жизненным циклом приложения.
