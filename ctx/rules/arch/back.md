...
Backend HomeCall включает встроенный сервер **WebSocket-сигналинга**, обеспечивающий обмен SDP и ICE-сообщениями между клиентами WebRTC.  
Компонент реализован как часть приложения `HomeCall_Back_App` и запускается в том же процессе Node.js.

- Канал `/ws/` обслуживается модулем `HomeCall_Back_Service_Signal_Server`.
- Сообщения маршрутизируются внутри комнат, определённых по кодовому слову.
- Сервер не хранит личные данные, лишь пересылает технические параметры соединения.
