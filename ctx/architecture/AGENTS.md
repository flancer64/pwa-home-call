...
- Shapes и boundaries могут ссылаться только на сущности, объявленные в `overview.md` и в `ctx/product/*` или `ctx/constraints/*`; они не создают новые роли или события и не повторяют реализационные подробности.
...
5. При ссылках используйте только `ctx/...`; внешние каталоги (например, `ctx/product/*`) не включаются без дополнительной инструкции.
