...
Вот сокращённый и уплотнённый вариант, соответствующий критериям ADSM. Текст декларативный, непротиворечивый, компактный и без избыточных пояснений.

---

## Файлы `output.md`

Агент обязан игнорировать файлы `output.md` где бы они ни находились. Файлы `output.md` являются сводом документов текущего каталога и просто дублируют все остальные документы в каталоге.
...
- ДомоЗвон не хранит и не обрабатывает персональные данные внутри приложения, поэтому модель-поток не включает в себя имена или другие идентификаторы пользователей.
- Любая идентификация участников происходит за пределами ДомоЗвон, через канал доставки ссылки приглашения.
