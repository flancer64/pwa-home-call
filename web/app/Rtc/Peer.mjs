...
export default function HomeCall_Web_Rtc_Peer({ HomeCall_Web_Env_Provider$: env, HomeCall_Web_Logger$: logger } = {}) {
  ...
  const configure = (newHandlers = {}) => {
    handlers = { ...handlers, ...newHandlers };
  };

  const onConnectionState = (handler) => subscribeConnectionState(handler);
  const onRemoteStream = (handler) => subscribeRemoteStream(handler);
